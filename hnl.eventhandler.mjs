...
class EventHandler {
  ...
  _initBodyResize() {
    if (typeof ResizeObserver !== 'undefined') {
      ...
      hnlLogger.warn(NAME, 'ResizeObserver not supported, using window resize fallback for bodyResize event');
  ...
  _initContentEvents() {
    ...
    if (imageCount === 0) {
      ...
      hnlLogger.info(NAME, 'No non-lazy images to load.');
      ...
      const checkComplete = () => {
        ...
        if (loaded === imageCount) {
          ...
          hnlLogger.info(NAME, `All non-lazy images loaded (${imageCount}/${totalImages} total).`);
          this._runListeners(['imgsLoaded']);
          this._states.imgsLoaded = true;
        }
      };
